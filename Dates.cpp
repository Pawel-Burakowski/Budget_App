...
string Dates::getTodaysDate()
{
    string date;
    SYSTEMTIME st;
    GetSystemTime(&st);
    string year = AuxillaryMethods::conversionFromIntToString(st.wYear);
    string month = AuxillaryMethods::conversionFromIntToString(st.wMonth);
    if (month.length() == 1)
        month = '0' + month;
    string day = AuxillaryMethods::conversionFromIntToString(st.wDay);
    if (day.length() == 1)
        day = '0' + day;
    date = year + '-' + month + '-' + day ;
    return date;
...
bool Dates::checkIfDateIsCorrect(string date)
{
...
int Dates::calculateTheNumberOfDaysInAMonth(int month, int year)
{
    switch(month)
    {
    case 1:
